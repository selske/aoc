...
public class Day12 {
    ...
    private static final String INPUT = "0 <-> 889, 1229, 1736\n" + //
                    "1 <-> 1, 480, 793, 1361\n" + //
                    "2 <-> 607\n" + //
                    "3 <-> 273, 422\n" + //
    ...
    private static final class Program {
        ...
        private final Set<Program> connections = new HashSet<>();
        ...
        public String toString() {
            return id + " <-> " + connections.stream().map((p) -> p.id + "").collect(Collectors.joining("-"));
        }
    ...
    public static void main(final String[] args) {
        ...
        reader.lines().forEach(line -> {
            ...
            Program program = programs.computeIfAbsent(id, (newId) -> new Program(newId));

            List<Program> connections = Arrays.stream(tokens[1].split("\\, *")) //
                            .map(Integer::valueOf) //
                            .map((connectionId) -> programs.computeIfAbsent(connectionId, (newId) -> new Program(newId))) //
                            .map(connection -> {
                                connection.connections.add(program);
                                return connection;
                            }) //
                            .collect(Collectors.toList());

            program.connections.addAll(connections);
        });

        System.out.println(programs.values());
        // Program initial = programs.get(0);
        // Set<Program> connectedPrograms = new HashSet<>();
        // connectedPrograms.add(initial);
        //
        // findConnections(initial, connectedPrograms);
        //
        // System.out.println(connectedPrograms.size());

        List<Program> ungroupedPrograms = new ArrayList<>(programs.values());
        int groupCount = 0;
        while (!ungroupedPrograms.isEmpty()) {
            Program initial = ungroupedPrograms.iterator().next();
            Set<Program> connectedPrograms = new HashSet<>();
            connectedPrograms.add(initial);

            findConnections(initial, connectedPrograms);

            ungroupedPrograms.removeAll(connectedPrograms);
            groupCount++;
        }
    ...
    private static void findConnections(final Program program, final Set<Program> connectedPrograms) {
        program.connections.stream() //
                        .filter(((Predicate<Program>) connectedPrograms::contains).negate()) //
                        .forEach(connection -> {
                            connectedPrograms.add(connection);
                            findConnections(connection, connectedPrograms);
                        });
    }
