...
public class Day8 {
    ...
    private static final String INPUT = "uz inc 134 if hx > -10\n" + //
                    "qin dec -300 if h <= 1\n" + //
    ...
    private static enum Action {
        inc(AtomicInteger::addAndGet), //
        dec((cur, inc) -> cur.addAndGet(inc * -1));

        private final BiConsumer<AtomicInteger, Integer> action;

        private Action(final BiConsumer<AtomicInteger, Integer> action) {
            this.action = action;
        }

        public void act(final AtomicInteger current, final Integer val) {
            action.accept(current, val);
        }
    }

    private static enum Eval {
        lt("<", (cur, val) -> cur.get() < val), //
        lte("<=", (cur, val) -> cur.get() <= val), //
        eq("==", (cur, val) -> cur.get() == val), //
        ne("!=", (cur, val) -> cur.get() != val), //
        gt(">", (cur, val) -> cur.get() > val), //
        gte(">=", (cur, val) -> cur.get() >= val);

        private final String token;
        private final BiFunction<AtomicInteger, Integer, Boolean> evalFunction;

        private Eval(final String token, final BiFunction<AtomicInteger, Integer, Boolean> evalFunction) {
            this.token = token;
            this.evalFunction = evalFunction;
        }

        public boolean evaluate(final AtomicInteger cur, final Integer val) {
            return evalFunction.apply(cur, val);
        }

        private static Eval getFromToken(final String token) {
            return Arrays.stream(values()) //
                            .filter(eval -> eval.token.equals(token)) //
                            .findFirst().get();
        }
    }
    ...
    public static void main(final String[] args) {
        ...
        AtomicInteger historicMax = new AtomicInteger(Integer.MIN_VALUE);

        final BufferedReader reader = new BufferedReader(new StringReader(INPUT));
        reader.lines() //
                        .forEach(line -> {
                            String[] tokens = line.split(" {1,}");
                            String varToAct = tokens[0];
                            AtomicInteger varToActValue = values.computeIfAbsent(varToAct, (x) -> new AtomicInteger(0));
                            Action action = Action.valueOf(tokens[1]);
                            Integer valueToAct = Integer.valueOf(tokens[2]);

                            String varToEvaluate = tokens[4];
                            AtomicInteger varToEvaluateValue = values.computeIfAbsent(varToEvaluate, (x) -> new AtomicInteger(0));
                            Eval eval = Eval.getFromToken(tokens[5]);
                            Integer valueToEval = Integer.valueOf(tokens[6]);

                            if (eval.evaluate(varToEvaluateValue, valueToEval)) {
                                action.act(varToActValue, valueToAct);
                                if (varToActValue.get() > historicMax.get()) {
                                    historicMax.set(varToActValue.get());
                                }
                            }
                        });

        Optional<Integer> max = values.values().stream().map(AtomicInteger::get).max(Integer::compareTo);
        System.out.println(max.get());
        System.out.println(historicMax);
    }
