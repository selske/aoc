...
public class Day20 {
    private static final String INPUT = // "p=<-6,0,0>, v=<3,0,0>, a=<0,0,0>\n" + "p=<-4,0,0>,
                                        // v=<2,0,0>, a=<0,0,0>\n" + "p=<-2,0,0>, v=<1,0,0>,
    ...
    private static final class Triple {
        ...
        public void add(final Triple other) {
            this.x += other.x;
            this.y += other.y;
            this.z += other.z;
        }
    ...
    private static final class Point {
        ...
        private final Triple location, velocity, acceleration;

        public Point(final int index, final Triple location, final Triple velocity, final Triple acceleration) {
            this.index = index;
        ...
        @Override
        public String toString() {
            return "p=<" + location + ">, v=<" + velocity + ">, a=<" + acceleration + ">";
        }
    }
    ...
    public static void main(final String[] args) {
        ...
        List<Point> points = new BufferedReader(new StringReader(INPUT)) //
                        .lines() //
                        .map(line -> {
                            Matcher matcher = pattern.matcher(line);
                            matcher.matches();
                            int i = 1;
                            Triple location = new Triple(Integer.valueOf(matcher.group(i++)), Integer.valueOf(matcher.group(i++)), Integer.valueOf(matcher.group(i++)));
                            Triple velocity = new Triple(Integer.valueOf(matcher.group(i++)), Integer.valueOf(matcher.group(i++)), Integer.valueOf(matcher.group(i++)));
                            Triple acceleration = new Triple(Integer.valueOf(matcher.group(i++)), Integer.valueOf(matcher.group(i++)), Integer.valueOf(matcher.group(i++)));
                            return new Point(index.getAndIncrement(), location, velocity, acceleration);
                        })//
    ...
    private static int part1(final List<Point> points) {
        return points.stream() //
                        .sorted((p1, p2) -> p1.getAccelerationDistance() - p2.getAccelerationDistance()) //
                        .findFirst()//
                        .get().index;
    }

    private static int part2(final List<Point> points) {
        for (int i = 0; i < 100; i++) {
            ...
            for (Point point : newPoints) {
                for (Iterator<Point> iterator = points.iterator(); iterator.hasNext();) {
                    ...
                    if (point.location.equals(point2.location) && !point.equals(point2)) {
                        iterator.remove();
                    }
                }
            }
