...
public class Day3 {
    ...
    private static void part1() {
        Set<Coordinate> visitedHouses = new HashSet<>();

        Coordinate coordinate = new Coordinate(0, 0);
        visitedHouses.add(coordinate);
        for (char c : INPUT.toCharArray()) {
            coordinate = getNewCoordinate(coordinate, c);
            visitedHouses.add(coordinate);
        }
    ...
    private static void part2() {
        ...
        List<Coordinate> coordinates = Arrays.asList(start, start);
        visitedHouses.add(start);

        char[] charArray = INPUT.toCharArray();
        for (int i = 0; i < charArray.length; i++) {
            char c = charArray[i];
            int coordinateIndex = i%2;
            Coordinate coordinate = coordinates.get(coordinateIndex);
            coordinate = getNewCoordinate(coordinate, c);
            coordinates.set(coordinateIndex, coordinate);
            visitedHouses.add(coordinate);
        }
    ...
    private static Coordinate getNewCoordinate(Coordinate coordinate, final char c) {
        switch (c) {
            case '>':
