...
public class Day6 {
    ...
    private static void part1() {
        Day6Rule part1Rule = (instruction, grid, x, y) -> {
            switch (instruction) {
                case "turn on":
                    grid[x][y] = 1;
                    break;
        ...
        int[][] grid = calculateGrid(part1Rule);
    ...
    private static void part2() {
        Day6Rule part2Rule = (instruction, grid, x, y) -> {
            int curVal = grid[x][y];
            switch (instruction) {
                case "turn on":
                    grid[x][y] += 1;
        ...
        int[][] grid = calculateGrid(part2Rule);
    ...
    private static int[][] calculateGrid(final Day6Rule rule) {
        ...
        new BufferedReader(new StringReader(INPUT)).lines() //
                .forEach(line -> {
                    ...
                    int index = 1;
                    String instruction = matcher.group(index++);
                    int fromX = Integer.valueOf(matcher.group(index++));
                    int fromY = Integer.valueOf(matcher.group(index++));
                    int toX = Integer.valueOf(matcher.group(index++));
                    int toY = Integer.valueOf(matcher.group(index++));



                    for (int x = fromX; x <= toX; x++) {
                        for (int y = fromY; y <= toY; y++) {
                            rule.execute(instruction, grid, x, y);
                        }
                    }
