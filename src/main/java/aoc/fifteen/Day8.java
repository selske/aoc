...
public class Day8 {
    ...
    private static final String INPUT = "\"\\xa8br\\x8bjr\\\"\"\n" +
            "\"nq\"\n" +
    ...
    private static void part1() {
        Integer codeLength = new BufferedReader(new StringReader(INPUT)).lines() //
                .map(String::length) //
                .collect(Collectors.summingInt(i -> i));


        Integer memorySize = new BufferedReader(new StringReader(INPUT)).lines() //
                .map(l -> l.replaceAll("\\\\x[0-9a-f][0-9a-f]", " ")) //
                .map(l -> l.replace("\\\\", " ")) //
                .map(l -> l.replace("\\", "")) //
                .map(String::length) //
                .collect(Collectors.summingInt(i -> i - 2));

        System.out.println(codeLength - memorySize);
    }

    private static void part2() {
        Integer encodedLength = new BufferedReader(new StringReader(INPUT)).lines() //
                .map((Function<String, String>) input -> "\"" + input.replace("\\", "\\\\").replace("\"", "\\\"") + "\"") //
                .map(String::length) //
                .collect(Collectors.summingInt(i -> i));


        Integer originalLength = new BufferedReader(new StringReader(INPUT)).lines() //
                .map(String::length) //
                .collect(Collectors.summingInt(i -> i));

        System.out.println(encodedLength - originalLength);
    }
